/**
 * ASHWIN KRISHNA — WORDPRESS INTERACTIVE JAVASCRIPT ENGINE
 * Crafted by 15+ Yrs Senior UI/UX Creative Technologist & WordPress Architect
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 10. INTERACTIVE CONTACT FORM (WORDPRESS AJAX INTEGRATION)
    initContactForm();
...
/* --------------------------------------------------------------------------
   10. INTERACTIVE CONTACT FORM (WITH WORDPRESS AJAX)
   -------------------------------------------------------------------------- */
function initContactForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (typeof ashwinAjax !== 'undefined' && ashwinAjax.ajax_url) {
            const formData = new FormData();
            formData.append('action', 'ashwin_contact');
            formData.append('nonce', ashwinAjax.nonce);
            formData.append('name', name);
            formData.append('email', email);
            formData.append('subject', subject);
            formData.append('message', message);

            fetch(ashwinAjax.ajax_url, {
                method: 'POST',
                body: formData
            })
            .then(res => res.json())
            .then(data => {
                ...
                if (data.success) {
                    ...
                    feedback.textContent = data.data.message || 'Failed to send message.';
                    feedback.className = 'form-feedback error';
                }
            })
            .catch(() => {
